...
function main()
    ...
    figures_path = "figures/PECUZAL_EMB_orbits_standard_map_k=2.5_multi_IC/eps$(eps)/date_$(today())_PANEL" # New folder for panel plot

    mkpath(figures_path)
    
    # Load all .dat files in data_path
    data_files = filter(f -> endswith(f, ".dat"), readdir(data_path))
    data_files = data_files[1:40:end]
    ...
    embedded_time_series = zeros(n_ics, Nf, 2)
    ...
    for (row_idx, dj) in enumerate(reverse(dj_range)) # Get both index and value
        for (col_idx, di) in enumerate(di_range)
            ...
            save_path = joinpath(data_path, "PECUZAL_EMB_RPC_n_ics$(n_ics)_eps$(eps)_di$(di)_dj$(dj).jld2")
            RPC = if isfile(save_path)
                ...
                for ic_idx in 1:n_ics
                    embedded_time_series = pecuzal_embedding(StateSpaceSet(time_series[ic_idx, :, :]); max_cycles=10)[1]
                    RP = custom_recurrence_plot(Matrix(embedded_time_series), eps; periodic=true, period=2π)
                    temp_RPC[ic_idx, 1:length(embedded_time_series)] = local_morans_I(RP; 
