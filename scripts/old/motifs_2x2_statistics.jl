    ...
    for motif_idx in 1:2^(2)
        ...
        for (i, (system_name, system, params)) in enumerate(systems)
            ...
            if i == num_systems
                ...
                binary_string = digits(motif_idx - 1, base=2, pad=2)

                # Group into chunks of L elements
                grouped_binary = [join(binary_string[i:min(i+1-1, end)]) for i in 1:1:length(binary_string)]
...
function main()
    ...
    probabilities = zeros(length(systems), resolution, 2^(2), m_max+1)  # Extra dimension for components
    ...
    for (i, system_tuple) in enumerate(systems)
        ...
        if system_name == "Randn"
            ...
            # Compute probabilities for each recurrence rate
             Threads.@threads for idx in 1:length(rrs)
                ...
                probabilities[i, idx, :, 1] = motifs_probabilities(RP, L; shape=:timepair, sampling=:random, sampling_region=:upper, num_samples=0.05)
            end

        elseif system_name[1:2] == "AR"
        ...
        for m in 1:m_max
            ...
            # Compute probabilities
             Threads.@threads for idx in 1:length(rrs)
                RP = RecurrenceMatrix(StateSpaceSet(time_series), GlobalRecurrenceRate(rrs[idx]); metric = Euclidean(), parallel = true)
                probabilities[i, idx, :, m+1] = motifs_probabilities(RP, L; shape=:timepair, sampling=:random, sampling_region=:upper, num_samples=0.05)
            end
        end
    end
