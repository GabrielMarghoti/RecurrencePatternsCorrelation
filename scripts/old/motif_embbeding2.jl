...
function main()
    ...
    # Systems to analyze
    systems = [
        ("Randn", nothing, nothing),
        ("AR 0.1", nothing, 0.1),  # AR model with memory parameter a = 0.1
        ("AR 0.9", nothing, 0.9),  # AR model with memory parameter a = 0.9
    ...
    # Output directory
    base_output_dir = "/home/gabrielm/figuras/pik/motif_embedding_analysis_Feb_04/resol$(resolution)_Nf$(Nf)_L$(L)_mMax$(m_max)"
    mkpath(base_output_dir)

    # Analyze each system
    probabilities =  zeros(length(systems), resolution, 2^(L^2), m_max+1)
    for (i, system_tuple) in enumerate(systems)
        system_name, system, params = system_tuple
        ...
        elseif occursin("AR", system_name)
            time_series = generate_ar_trajectory(params, Nf)  # Generate AR trajectory
        ...
        elseif occursin("Sine", system_name)
            time_series = sin.(2π*params[1]*range(1,Nf))  # Generate AR trajectory
            ...
            time_series= trajectory[:, 1]


            τ = find_optimal_delay(trajectory, max_delay)
            print(" optimal tau: ", τ)
            Threads.@threads for idx in 1:length(rrs)
        ...
        for m in 1:m_max
            ...
            if m == 1
                embedded_trajectory = time_series
            else
                embedded_trajectory = embed(time_series, m, τ)
            end

            # Calculate microstate probabilities for each recurrence rate
            Threads.@threads for idx in 1:length(rrs)
                probabilities[i, idx, :, m+1] = calculate_microstate_probabilities(embedded_trajectory, L, rrs[idx])
            end
        end
